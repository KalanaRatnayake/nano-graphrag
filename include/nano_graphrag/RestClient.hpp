...
class RestClient {
 public:
  ...
  void set_ssl_verify(bool v) { ssl_verify_ = v; }
  void set_auth_bearer(const std::string& token) { auth_type_ = "Bearer"; api_key_ = token; }
  void set_auth_type(const std::string& t) { auth_type_ = t; }

  Poco::JSON::Object::Ptr post_json(Poco::JSON::Object& body_json, const std::string& uri) {
    Poco::URI uri_obj(uri);

    std::ostringstream body_stream; body_json.stringify(body_stream);
    ...
    if (uri_obj.getScheme() == "https") {
      ...
      if (ssl_verify_) params.caLocation = "/etc/ssl/certs";
    ...
    if (response.getStatus() != Poco::Net::HTTPResponse::HTTP_OK) {
      throw Poco::Net::NetException("HTTP Error: " + std::to_string(response.getStatus()) + " " + response.getReason());
    }
